...
namespace IngameScript
{
    public class Program : MyGridProgram
    {
        ...
        string CustomData = "";
        int PitDiameter = 0;
        int NumOfHorizontalPistons = 0;
        float PISTON_LENGTH_IN_METERS = 5.16;  //  7.5m - 2.34m = 5.16m long
        ...
        public void Main(string argument, UpdateType updateType)
        {
            ...
            IMyMotorAdvancedStator v1 = null;
            GridTerminalSystem.GetBlocksOfType<IMyMotorAdvancedStator>(l1, filterThis);
            if (l1.Count == 0)
            {
            ...
            else
            {
                ...
                {
                    if (l1[i].CustomName == "Advanced Rotor")
                    {
                        v1 = (IMyMotorAdvancedStator)l1[i];
                        break;
                    }
                }
                if (v1 == null)
            ...
            {
                GridTerminalSystem.GetBlockGroupWithName("pit horizontal").GetBlocksOfType<IMyPistonBase>(v2, filterThis);
                if (v2.Count == 0)
                {
            ...
            {
                v2[i].MinLimit = (float)15;
                v2[i].MaxLimit = (float)15;
                pistonsHorizontalCurrentPosition = getExtraFieldFloat(v2[i], "Current position: (\\d+\\.?\\d*)");
            }
        }
